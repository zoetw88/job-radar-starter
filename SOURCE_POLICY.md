...
The scanner rejects a source above its configured job-count limit inside the
producer process, before normalized `Job` objects or a large IPC result are
built. The HTTP client also caps response bytes before JSON decoding. These are
safety ceilings, not permission to request the maximum volume continuously.
...
The repository does not bundle or recommend automated access to LinkedIn,
Indeed, Google Jobs, 104, or JobSpy. In short: no bundled `LinkedIn`, no bundled
`Indeed`, no bundled `Google Jobs`, no bundled `104`, and no bundled `JobSpy`
scraper. Their absence is intentional: permission for private access methods
has not been established for a reusable public tool.

Cookie warming, browser impersonation, login or session reuse, undocumented
endpoints, access-control bypass, and TLS verification downgrade are outside
the public project's boundary.
...
The legacy `score`/`run --ai-command` protocol excludes the resume path, resume contents, prior summaries, risks, and application state. It does send structured skills, preferences, and company boundaries together with public job data to the developer-selected executable.

The compact `review --provider-command` protocol has a narrower disclosure boundary: it sends public job fields and bounded evidence from the public posting, and does not send profile-derived preferences, scoring rules, summaries, or risks.

For either protocol, if the selected executable calls a third party, the developer is responsible for disclosure, retention, sharing, training-use, and deletion behavior at that provider.
