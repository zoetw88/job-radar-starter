...
- [x] No maintainer resume, preferences, applications, credentials, scans, or
  historical Git objects exist in the clean-root publication candidate.
...
- [x] Publication content is prepared as one clean root commit with no parent;
  it does not inherit the personalized objects reachable from the old `main`.
- [ ] With explicit approval, replace the remote history from the clean-root
  branch and require hosted checks before treating the release as complete.
...
- [x] Legacy `score`/`run --ai-command` sends structured preferences and public
  job data but does not read resume contents automatically; compact
  `review --provider-command` sends public posting evidence only.
